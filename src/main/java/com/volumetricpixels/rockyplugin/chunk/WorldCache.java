...
public class WorldCache {
	 ...
	 */
	private final static int[] CRC32_TABLE = { 0x00000000, 0x77073096,
	...
	/**
	 * Each world has his own cache of chunk
	 */
	private Map<String, ChunkCacheEntry> chunkCache = new HashMap<String, ChunkCacheEntry>();
	/**
	 * The list of chunk a player hash has
	 */
	private Map<String, Map<String, ChunkCacheEntry>> playerCache = new HashMap<String, Map<String, ChunkCacheEntry>>();

	/**
	 * Gets the current cache of a chunk, if the chunk doesn't have a cache,
	 * creates a new entry for the cache
	 * 
	 * @param x
	 ...
	 */
	public synchronized ChunkCacheEntry getChunk(double x, double z) {
		String key = x + "." + z;
		if (!chunkCache.containsKey(key)) {
			chunkCache.put(key, new ChunkCacheEntry());
		}
	 ...
	 * @return the chunk cache structure
	 */
	public synchronized ChunkCacheEntry getPlayerChunk(String player, double x,
			double z) {
		String key = x + "." + z;
		if (!playerCache.containsKey(player)) {
			playerCache.put(player, new HashMap<String, ChunkCacheEntry>());
		}
		Map<String, ChunkCacheEntry> cache = playerCache.get(player);
		if (!cache.containsKey(key)) {
			cache.put(key, new ChunkCacheEntry());
		}
	 ...
	 */
	public static int calculateHash(byte[] buffer) {
		int crc = 0xffffffff;
		for (byte b : buffer) {
